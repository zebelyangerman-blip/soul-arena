...
(function(){
  ...
  function buildExplanation(context={}){
    ...
    if(engine?.finalRatingPolicy?.gapGuardApplied){
      const fatigueGuard=Boolean(engine?.finalRatingPolicy?.fatigueGapGuardApplied);
      bullets.push(fatigueGuard
        ? 'Защита шкалы силы сработала: при разрыве 12+ состояние «Ранен» само по себе не может перевернуть бой без подтверждённого специального контра или обхода способности.'
        : 'Защита шкалы силы сработала: обычные бонусы не получили права перевернуть большой разрыв рейтинга без подтверждённого специального контра или обхода способности.');
    }
